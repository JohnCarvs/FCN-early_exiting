...
%%%%%%%% Understanding the Interaction Between Deep Supervision
...
\icmlauthor{Lucas de Oliveira Ferreira}{ufmg}
\end{icmlauthorlist}

\icmlaffiliation{ufmg}{Universidade Federal de Minas Gerais, Belo Horizonte, Brazil}

% REQUIRED in [accepted] mode: at least one corresponding author.
% Replace the placeholder e-mail below with the real one.
\icmlcorrespondingauthor{João Vítor de Carvalho Silva}{joaocarvs@ufmg.br}
\icmlcorrespondingauthor{Lucas de Oliveira Ferreira}{lucasoferreira43@ufmg.br}
...
\section{Experiments}
    ...
    Following the original FCN protocol~\cite{ref_fcn}, our main experiments train
    on the Semantic Boundaries Dataset (SBD) \texttt{train} split and validate on
    the held-out \texttt{seg11valid} split (the standard FCN validation split,
    non-overlapping with training), over the 21 PASCAL classes (20 objects plus
    background). PASCAL VOC 2011 enters only in the fine-tuning transfer
    experiment (Table~\ref{tab:voc}), where the best SBD model is fine-tuned and
    evaluated on it. The encoder is initialized from ImageNet-pretrained VGG-16. Images
    are processed at their native resolution (mean-RGB subtraction only, no
    resizing or cropping), so the batch size is~1. We train for 200 epochs with
    per-pixel cross-entropy loss (void pixels ignored) and the Adam optimizer
    (learning rate $10^{-4}$, $\beta_1{=}0.5$, $\beta_2{=}0.999$), reporting mIoU
    and mean per-class pixel accuracy with validation at each epoch. Experiments
    ran either on a single NVIDIA RTX 6000 (48\,GB) or a single NVIDIA Titan X
    (12\,GB).
...
\label{sec:results}
    ...
    and FCN-8s + s16 with $\lambda=0.1$) and fine-tuned them on the VOC 2011 dataset, using the weights trained on SBD. Although absolute values remain below those reported in the original work~\cite{ref_fcn}, the trend holds: the auxiliary-exit model keeps its advantage over the baseline. The effect is thus not specific to SBD.
    
    \begin{table}[t]
\caption{Fine-tuning on VOC 2011: the advantage of the auxiliary exit persists.
Metrics are computed on the VOC 2011 validation set (best epoch).}
\label{tab:voc}
\vskip 0.15in
\begin{center}
\begin{small}
...
% ---- Bibliography (BibTeX: natbib + icml2025.bst, APA author-year) ----
\bibliography{main_icml2025}
